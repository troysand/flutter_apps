...
class _HomePageState extends State<HomePage> {
  ...
  Widget _selectedCoinDropdown() {
    ...
    List<DropdownMenuItem<String>> coinsListItems = coinsList
        .map(
          (e) => DropdownMenuItem(
            value: e,
            child: Text(
    ...
    return DropdownButton(
      value: _selectedCoin,
      items: coinsListItems,
      onChanged: (v) {
        setState(() {
          _selectedCoin = v.toString();
        });
      },
      ...
      builder: (BuildContext c, AsyncSnapshot snapshot) {
        if (snapshot.hasData) {
          ...
          return Column(
            ...
            children: [
              GestureDetector(
                child: _coinImageWidget(coinData["image"]["large"]),
                onDoubleTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (BuildContext bc) {
                        return DetailsPage(
                          rates: coinData["market_data"]["current_price"],
                        );
                      },
                    ),
                  );
              ...
              _descriptionCardWidget(coinData["description"]["en"]),
            ],
          );
        } else {
          return const Center(
